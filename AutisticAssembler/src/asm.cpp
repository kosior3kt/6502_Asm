...
namespace
{
   ...
   void generateStartSequence(const uint8_t LSB_forJump = 0x00, const uint8_t MSB_forJump = 0x80);
}
...
namespace Asm
{
   ...
   {
      ///TODO: add support for comments that are not on the beggining of the line
...
namespace
{
   ...
   ///!!!THIS THING HAS DEFAULT VALUES!!!
   void generateStartSequence(const uint8_t LSB_forJump, const uint8_t MSB_forJump)  
   {
      ::fillAt((1024*64 - 4), 0x4C);
      ::fillAt((1024*64 - 3), LSB_forJump);
      ::fillAt((1024*64 - 2), MSB_forJump);
